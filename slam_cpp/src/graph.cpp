...
void Graph::createPose(const std::shared_ptr<Pose>& measurement)
{
    ...
    ceres::CostFunction* cost_function =
        new ceres::AutoDiffCostFunction<PoseErrorFunction, 3, 3, 3, 3>(new PoseErrorFunction);

    m_problem.AddResidualBlock(cost_function, nullptr, prev->second.m_data, curr->second.m_data, meas->m_data);
